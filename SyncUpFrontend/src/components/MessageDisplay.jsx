...
const MessageDisplay = ({ messages, selectedChannel, channels, name }) => {
  ...
  const initialScrollDone = useRef(false); // Track if initial scroll has happened
  ...
  //     scrollToBottom(); // Scroll only on the first render
  ...
  useEffect(() => {
    if (!initialScrollDone.current) {
      ...
      scrollToBottom(); // Scroll on messages change
    }
